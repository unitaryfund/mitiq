...
def _check_scalable(input_circuit: Circuit) -> None:
    """Raises an error if the input circuit cannot be scaled by inserting identity layers.
...
def _calculate_id_layers(input_circuit_depth: int, scale_factor: float) -> List[int]:
    """Returns a list of integers that describes the number of identity layers to be
    inserted after each layer of the input circuit. 
    ...
    Returns:
        [num_uniform_layers, num_partial_layers] : A list of the number of uniform
            identity layers to be inserted after each moment in the input_circuit and
            a number of partial layers to be inserted after some random moments to be
            able to achieve the intended scale factor.
    """
    ...
    if np.isclose(int_scale_factor, scale_factor):
        ...
        # find partial layers by approximating closest to the desired scale_factor
        num_partial_layers=int(input_circuit_depth*(scale_factor-1-num_uniform_layers))
...
def insert_id_layers(input_circuit: Circuit, scale_factor: float)-> Circuit:
    ...
    if num_partial_layers != 0:
       random_moment_indices = np.random.randint(input_circuit_depth,size = num_partial_layers).tolist()
    ...
    id_layer = cirq.Moment(cirq.ops.I.on_each(circuit_qubits))
