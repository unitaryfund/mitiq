...
import pytest
from mitiq.zne.scaling.identity_insertion import UnscalableCircuitError, _calculate_id_layers, insert_id_layers
...
def test_id_layers_whole_scale_factor(scale_factor):
    ...
    expected_circ = Circuit([ops.H.on_each(*qreg)], [ops.I.on_each(*qreg)] * num_layers, [ops.CNOT.on(qreg[0], qreg[1])], [ops.X.on(qreg[2])], [ops.I.on_each(*qreg)] * num_layers, [ops.TOFFOLI.on(*qreg)], [ops.I.on_each(*qreg)] * num_layers)
...
def test_scale_with_intermediate_measurements_raises_error():
    ...
    with pytest.raises(UnscalableCircuitError, match='Circuit contains intermediate measurements'):
...
def test_scaling_with_terminal_measurement():
    ...
    expected_circ = Circuit([ops.H.on(qbit)], [ops.I.on(qbit)] * 2, [ops.T.on(qbit)], [ops.I.on(qbit)] * 2, [ops.measure(qbit)])
    assert _equal(scaled_circ, expected_circ)
...
@pytest.mark.parametrize('intended_scale_factor', (1, 1.1, 1.3, 1.7, 1.9, 2, 3.1, 3.6, 3.9, 3, 4, 5, 6))
