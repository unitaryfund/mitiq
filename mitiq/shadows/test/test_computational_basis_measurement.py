...
def test_shadow_measure_with_executor_output_dimensions(n_qubits: int):
    qubits = cirq.LineQubit.range(n_qubits)
    circuit = simple_test_circuit(qubits)
    shadow_outcomes, pauli_strings = shadow_measure_with_executor(circuit, executor, n_total_measurements)
    assert shadow_outcomes.shape == (n_total_measurements,
                                     n_qubits), f"Shadow outcomes have incorrect shape, expected {(n_total_measurements, n_qubits)}, got {shadow_outcomes.shape}"
    assert pauli_strings.shape == (
    n_total_measurements,), f"Pauli strings have incorrect shape, expected {(n_total_measurements, n_qubits)}, got {pauli_strings.shape}"
    assert len(pauli_strings[
                   0]) == n_qubits, f"Pauli strings have incorrect number of characters, expected {n_qubits}, got {len(pauli_strings[0])}"
...
def test_shadow_measure_with_executor_output_types(n_qubits: int):
    ...
    assert shadow_outcomes[
               0].dtype == int, f"Shadow outcomes have incorrect dtype, expected int, got {shadow_outcomes.dtype}"
    assert isinstance(pauli_strings[0],
                      str), f"Pauli strings have incorrect dtype, expected str, got {pauli_strings.dtype}"
...
def test_shadow_measure_with_executor_time_growth(n_qubits: int):
    ...
    for i in range(1, len(times)):
        assert times[i] / times[i - 1] == pytest.approx(measurements[i] / measurements[i - 1], rel=0.2)
