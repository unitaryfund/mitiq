...
def rb_circuits(n_qubits: int, num_cliffords: List[int], trials: int) -> List[Circuit]:
    ...
    for num in num_cliffords:
        ...
        if n_qubits == 1:
            ...
            qubit2 = NamedQubit("1")
            cfd_matrices = _two_qubit_clifford_matrices(qubit1, qubit2, CLIFFORDS)
            rb_circuits = [
                _random_two_q_clifford(qubit1, qubit2, num, cfd_matrices, CLIFFORDS)
                for _ in range(trials)
            ]
        else:
            raise ValueError(
                "Only generates RB circuits on one or two " f"qubits not {n_qubits}."
            )
    return rb_circuits
