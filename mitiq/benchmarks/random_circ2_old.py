...
def rand_benchmark_zne(
    ...
    for circuit_number in range(trials):
        ...
        hardware_state = np.diag(dsim.simulate(
            circuit.with_noise(cirq.depolarize(p=depolarizing_noise_strength))
        ).final_density_matrix)
        hardware_probs = np.abs(hardware_state)**2
        ...
        noisy_states = [
            np.diag(dsim.simulate(noisy).final_density_matrix)
            for noisy in noisy_circuits
        ]
        noisy_probs = [np.abs(noisy_state)**2 for noisy_state in noisy_states]
