...
@pytest.mark.parametrize("n_qubits", (1, 2))
@pytest.mark.parametrize("return_type", ("pyquil", "qiskit"))
def test_converted_rb_circuits(n_qubits, return_type):
    depth = 10

    # test single qubit RB
    for trials in [2, 3]:
        circuits = generate_converted_rb_circuits(
            n_qubits=n_qubits,
            num_cliffords=depth,
            trials=trials,
...
@pytest.mark.parametrize("return_type", ("pyquil", "qiskit"))
def test_converted_random_benchmarks(scale_noise, fac, return_type):
    trials = 3
    circuits = generate_converted_rb_circuits(
        n_qubits=2, num_cliffords=4, trials=trials, return_type=return_type
    )
    # noise = 0.01
    # obs = np.diag([1, 0, 0, 0])
...
def test_converted_random_one_qubit_identity(num_cliffords, return_type):
    qp = random_one_qubit_identity_circuit(
        num_cliffords=num_cliffords, return_type=return_type
    )
    # Place for a unified executor?
    # wvf = qp.final_state_vector()
    # zero_prob = abs(wvf[0] ** 2)
    # assert np.isclose(zero_prob, 1)
    assert return_type in qp.__module__