...
from mitiq.interface.mitiq_qiskit.qiskit_utils import execute, execute_with_shots, execute_with_noise, execute_with_shots_and_noise, initialized_depolarizing_noise, sample_bitstrings, compute_expectation_value_on_noisy_backend
...
def test_execute_with_shots():
    ...
    second_circ = QuantumCircuit(1)
    second_circ.x(0)
    expectation_value = execute_with_shots(circuit=second_circ, obs=ONE_QUBIT_GS_PROJECTOR, shots=SHOTS)
...
def test_execute_with_depolarizing_noise_single_qubit():
    ...
    expectation_value = execute_with_noise(circuit=single_qubit_circ, obs=ONE_QUBIT_GS_PROJECTOR, noise_model=initialized_depolarizing_noise(NOISE))
...
def test_execute_with_depolarizing_noise_two_qubit():
    ...
    noiseless_exp_value = 1.0
    expectation_value = execute_with_noise(circuit=two_qubit_circ, obs=TWO_QUBIT_GS_PROJECTOR, noise_model=initialized_depolarizing_noise(NOISE))
    assert expectation_value < noiseless_exp_value
...
def test_execute_with_shots_and_depolarizing_noise_single_qubit():
    ...
    single_qubit_circ.z(0)
    noiseless_exp_value = 1.0
    expectation_value = execute_with_shots_and_noise(circuit=single_qubit_circ, obs=ONE_QUBIT_GS_PROJECTOR, noise_model=initialized_depolarizing_noise(NOISE), shots=SHOTS)
    assert expectation_value < noiseless_exp_value
...
def test_execute_with_shots_and_depolarizing_noise_two_qubit():
    ...
    two_qubit_circ.cx(0, 1)
    noiseless_exp_value = 1.0
    expectation_value = execute_with_shots_and_noise(circuit=two_qubit_circ, obs=TWO_QUBIT_GS_PROJECTOR, noise_model=initialized_depolarizing_noise(NOISE), shots=SHOTS)
    assert expectation_value < noiseless_exp_value
...
def test_circuit_is_not_mutated_by_executors():
    ...
    execute_with_shots_and_noise(circuit=single_qubit_circ, obs=ONE_QUBIT_GS_PROJECTOR, noise_model=initialized_depolarizing_noise(NOISE), shots=SHOTS)
    assert single_qubit_circ.data == expected_circuit.data
    assert single_qubit_circ == expected_circuit
    execute_with_noise(circuit=single_qubit_circ, obs=ONE_QUBIT_GS_PROJECTOR, noise_model=initialized_depolarizing_noise(NOISE))
    assert single_qubit_circ.data == expected_circuit.data
    assert single_qubit_circ == expected_circuit
...
def test_sample_bitstrings():
    ...
    measurement_result = sample_bitstrings(circuit=two_qubit_circ, backend=None, noise_model=initialized_depolarizing_noise(0), shots=5)
...
def test_sample_bitstrings_with_measure_all():
    ...
    two_qubit_circ = QuantumCircuit(2)
    two_qubit_circ.cx(0, 1)
    measurement_result = sample_bitstrings(circuit=two_qubit_circ, backend=None, noise_model=initialized_depolarizing_noise(0), shots=2, measure_all=True)
...
def test_sample_bitstrings_with_backend():
    ...
    two_qubit_circ = QuantumCircuit(2)
    two_qubit_circ.cx(0, 1)
    measurement_result = sample_bitstrings(circuit=two_qubit_circ, backend=FakeLima(), shots=5, measure_all=True)
...
def test_sample_bitstrings_error_message():
    ...
    with pytest.raises(ValueError, match='Either a backend or a noise model'):
        sample_bitstrings(circuit=two_qubit_circ, shots=5)
...
def test_compute_expectation_value_on_noisy_backend_with_noise_model():
    ...
    qiskit_circuit.h(0)
    noiseless_expval = compute_expectation_value_on_noisy_backend(qiskit_circuit, obs, noise_model=initialized_depolarizing_noise(0))
    ...
    expval = compute_expectation_value_on_noisy_backend(qiskit_circuit, obs, noise_model=initialized_depolarizing_noise(0.01))
...
def test_compute_expectation_value_on_noisy_backend_with_qiskit_backend():
    ...
    qiskit_circuit = QuantumCircuit(1)
    qiskit_circuit.h(0)
    expval = compute_expectation_value_on_noisy_backend(qiskit_circuit, obs, backend=FakeLima())
    assert isinstance(expval, complex)
    assert np.isclose(np.imag(expval), 0.0)
    assert 0.9 < np.real(expval) < 1.0