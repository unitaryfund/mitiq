...
def executor_batched(circuits, **kwargs) -> List[float]:
    return [float(v) for v in np.full(shape=(len(circuits),), fill_value=kwargs.setdefault('return_value', 0.0))]
...
def test_run_executor_nonidentical_pyquil_programs(batch_size):
    ...
    assert collector.can_batch
    circuits = [pyquil.Program(pyquil.gates.X(0)), pyquil.Program(pyquil.gates.H(0))] * 10
    results = collector.run(circuits)
...
def test_run_executor_all_unique(ncircuits, batch_size):
    ...
    circuits = [cirq.testing.random_circuit(qubits=4, n_moments=10, op_density=1, random_state=random_state) for _ in range(ncircuits)]
    results = collector.run(circuits)
    assert np.allclose(results, np.zeros(ncircuits))
...
def test_run_executor_preserves_order(s, b):
    ...
    assert collector.can_batch
    circuits = [cirq.Circuit(cirq.H(cirq.LineQubit(0))), cirq.Circuit([cirq.H(cirq.LineQubit(0))] * 2)]
...
@pytest.mark.parametrize('execute', [executor_batched, executor_batched_unique, executor_serial_unique, executor_serial_typed, executor_serial, executor_pyquil_batched])
@pytest.mark.parametrize('obs', [PauliString('X'), PauliString('XZ'), PauliString('Z')])
