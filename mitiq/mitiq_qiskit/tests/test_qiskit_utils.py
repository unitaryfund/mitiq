...
def test_qs_wvf_sampling_sim():
    ...
    circ = qiskit.QuantumCircuit(qiskit.QuantumRegister(1))
    expectation_value = qs_wvf_sampling_sim(circ=circ, obs=observable, shots=shots)
...
def test_qs_wvf_sampling_sim_error():
    ...
    circuit for the "qs_wvf_sampling_sim" executor
    """
...
def test_qs_noisy_sampling_sim_error():
    """ Tests that an error is raised when a classical register is present in a Qiskit
    circuit for the "qs_noisy_sampling_sim" executor
    """
    circ = qiskit.QuantumCircuit(qiskit.QuantumRegister(1), qiskit.ClassicalRegister(1))
    with pytest.raises(ValueError, match="This executor only works on programs with no classical bits."):
