...
# Test the documentation examples about factories
...
def noise_to_expval_to_clip(noise_level: float) -> float:
      """Linear model with a larger zero-noise limit."""
      return noise_to_expval(noise_level) + 1

def test_doc_factories_one():
    """Test doc on factories. First example."""
    ...
    zn_limit = fac.reduce()
    assert np.isclose(zn_limit, 0.5)

def test_doc_factories_two():
    """Test doc on factories. Second example."""
    NOISE_LEVELS = [1.0, 2.0, 3.0]
    fac = LinearFactory(NOISE_LEVELS)
...
def test_doc_factories_three():
    """Test doc on factories. Third example.
    A custom factory is defined and used.
    """
    class MyFactory(BatchedFactory):
        ...
        def __init__(
            ...
            """
            Args:
                scale_factors: The noise scale factors at which expectation
    ...
    # Test custom factory
    NOISE_LEVELS = [1.0, 2.0, 3.0]
    fac = MyFactory(NOISE_LEVELS, min_expval=-1.0, max_expval=1.0)
    run_factory(fac, noise_to_expval)
    assert np.isclose(fac.reduce(), 0.5)
    run_factory(fac, noise_to_expval_to_clip)
