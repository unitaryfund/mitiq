...
Information in the docs can be added as markdown (`.md`/`.myst.md`) files, as
...
The main table of contents (TOC) file for the docs is `index.myst.md`. It includes `guide\guide.myst.md` and `apidoc.myst.md`, among other files. To add a new file to the base TOC, make sure it gets listed in the `toctree` directive like this:
````
```{toctree}
---
maxdepth: 2 
...
To include `.md` files outside of the documentation `source` directory, you can add a stub `*.myst.md` file to the toctree inside the `docs/source` directory that contains:
...
To add information in the guide, please add markdown (`.md`) or MyST markdown (`.myst.md`) files to the `docs/guide` directory.
...
If you have a notebook you want to add, and want to automatically convert it from the `.ipynb` to `.myst.md`, you can use a great Python command line tool called [jupytext](https://jupytext.readthedocs.io/en/latest/index.html).
...
```{tip}
There is a [sample markdown formatted notebook in the `examples` directory](./examples/template.myst.md) for you to take a look at as you write your own!
```

### Automatically add information from the API docs

New modules, classes and functions can be added by listing them in the appropriate file (such as `apidoc.myst.md` or a child), e.g.,
...
source code, and to the API page `apidoc.myst.md`.
```
