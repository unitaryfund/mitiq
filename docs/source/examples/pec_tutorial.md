...
PEC samples many circuits according to the input representations. For each unitary, $U$ in the circuit with representation

$$
U = \sum_i a_i G_i,
$$
where channel $G_i$ replaces $U$ with (normalized) probability $|a_i|$.
...
For each call of `execute`, we have a collection of expectation values and signs. We combine them as per

$$
\frac{\eta}{M} \sum_{i=1}^M \sigma_i E_i
$$
...
![PEC results](https://i.ibb.co/m4v1201/download.png)

We can see that PEC on average improves the expectation value at each depth.