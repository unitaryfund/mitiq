...
We now define an [executor](executors.myst) which simulates a backend with depolarizing noise and computes the survival probability of the $|00\rangle$ state.
...
{func}`.find_optimal_representation()` function as shown the section [What additional options are available in PEC?](pec-3-options.myst).
