...
The `html`, `latex` and `pdf` files will be automatically created in the
`docs/build` folder with similar commands.

Note that `make html` reads the `make.bat` file in the `docs/` folder; this was
generated by `sphinx-quickstart`, used to generate the documentation in the
first place. The makefile automatically runs a script that implements the
explicit command `sphinx-build -b html source build`.
